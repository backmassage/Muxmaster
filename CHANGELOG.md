...
- **Content-aware prefiltering (`--tune auto|none|film|grain|anime`).** Injects a CPU prefilter into the software-decode chain to raise quality-per-bit on raw Blu-ray sources: `film`→`hqdn3d=1.5:1.5:6:6` and `grain`→`hqdn3d=4:4:9:9` (denoise — the biggest measured lever, e.g. −21% size at −0.15 VMAF on grain), `anime`→`gradfun=1.2:16` (deband). Because CPU filters cannot enter a hardware VAAPI surface chain (`denoise_vaapi` is unimplemented on radeonsi), an active prefilter forces software decode in VAAPI mode. `anime` also nudges QP/CRF +1 (flat cels tolerate it; deband handles banding). Explicit `--tune none` keeps the existing pipeline byte-for-byte.
